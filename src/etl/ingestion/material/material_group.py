...
url = f"{BASE_URL}/modulo-material/1_consultarGrupoMaterial"
...
try:
    ...
    log_error(log, "upsert_failed", table="material_group", rows_done=inserted, error=str(exc))
    raise
finally:
    conn.close()
log_info(log, "upsert_ok", table="material_group", rows=inserted)
