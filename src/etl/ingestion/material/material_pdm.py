...
log = get_json_logger("material_pdm")
...
try:
    ...
    log_error(log, "upsert_failed", table="material_pdm", rows_done=inserted, error=str(exc))
    raise
finally:
    conn.close()
log_info(log, "upsert_ok", table="material_pdm", rows=inserted)
